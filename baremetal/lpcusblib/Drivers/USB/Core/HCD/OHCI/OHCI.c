...
{
	if ( USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus) {	/* If device is connected */
		*DeviceSpeed =
			( USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_LowSpeedDeviceAttached) ? LOW_SPEED : FULL_SPEED;
...
HCD_STATUS HcdRhPortReset(uint8_t HostID)
{
	...
	USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortResetStatus;	/* SetPortReset */
	/* should have time-out */
	while ( USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_PortResetStatus) {}

	USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortResetStatusChange;/* Clear Port Reset Status Change */
...
{
	USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PowerEnableStatus;/* SetPortEnable */
...
{
	USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_CurrentConnectStatus;	/* ClearPortEnable */
...
HCD_STATUS HcdInitDriver(uint8_t HostID)
{
	...
	USB_REG(HostID)->OTGStCtrl = 0x3;					/* ??? */
#elif defined(__LPC177X_8X__) || defined(__LPC407X_8X__)
	USB_REG(HostID)->OTGStCtrl = 0x1;					/* Port 1 is host */
#endif
...
HCD_STATUS HcdCancelTransfer(uint32_t PipeHandle)
{
	...
	USB_REG(HostID)->HcInterruptStatus = HC_INTERRUPT_StartofFrame;
	while ( !(USB_REG(HostID)->HcInterruptStatus & HC_INTERRUPT_StartofFrame) )/* TODO Should have timeout */
							  ...
							  uint8_t *const buffer)
{
	...
	USB_REG(HostID)->HcCommandStatus |= HC_COMMAND_STATUS_ControlListFilled;
						   ...
						   uint16_t *const pActualTransferred)
{
	...
	if ( IsIsoEndpoint(EdIdx) ) {	/* Iso Transfer */
		...
		if (HcdED(EdIdx)->ListIndex == BULK_LIST_HEAD) {
			USB_REG(HostID)->HcCommandStatus |= HC_COMMAND_STATUS_BulkListFilled;
		}
	}
...
void HcdIrqHandler(uint8_t HostID)
{
	...
	IntStatus = USB_REG(HostID)->HcInterruptStatus;
	/* Clear status after read immediately.
		 Then it will be able to record a new status. */
	USB_REG(HostID)->HcInterruptStatus = IntStatus;/* Clear HcInterruptStatus */
	IntStatus &= USB_REG(HostID)->HcInterruptEnable;
	if (IntStatus == 0) {
		return;
	}

	/* disable all interrupt for processing */
	USB_REG(HostID)->HcInterruptDisable = HC_INTERRUPT_MasterInterruptEnable;

	/* Process RootHub Status Change */
	if (IntStatus & HC_INTERRUPT_RootHubStatusChange) {
		for(;USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_StatusChangeMask;){
			/* only 1 port/host --> skip to get the number of ports */
			if (USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_ConnectStatusChange) {
				if (USB_REG(HostID)->HcRhStatus & HC_RH_STATUS_DeviceRemoteWakeupEnable) {	/* means a remote wakeup event */

				}
				else {}

				USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_ConnectStatusChange;	/* clear CSC bit */
				OHciRhStatusChangeIsr(HostID, USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus);
			}

			if (USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_PortEnableStatusChange) {
				USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortEnableStatusChange;	/* clear PESC */
			}

			if (USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_PortSuspendStatusChange) {
				USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortSuspendStatusChange;			/* clear PSSC */
			}

			if (USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_OverCurrentIndicatorChange) {	/* Over-current handler to avoid physical damage */
				USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_OverCurrentIndicatorChange;			/* clear OCIC */
			}

			if (USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_PortResetStatusChange) {
				USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortResetStatusChange;			/* clear PRSC */
			}
		}
	}
...
#endif
	USB_REG(HostID)->HcInterruptEnable = HC_INTERRUPT_MasterInterruptEnable;
...
static __INLINE HCD_STATUS OHciHostInit(uint8_t HostID)
{
	...
	USB_REG(HostID)->HcControl = 0;
	OHciHostOperational(HostID);/* have to turn HC to operational mode before setting up below registers*/

	USB_REG(HostID)->HcFmInterval = HC_FMINTERVAL_DEFAULT;
	USB_REG(HostID)->HcPeriodicStart = PERIODIC_START;

	USB_REG(HostID)->HcControlHeadED = (uint32_t) &(ohci_data[HostID].staticEDs[CONTROL_LIST_HEAD]);
	USB_REG(HostID)->HcBulkHeadED = (uint32_t) &(ohci_data[HostID].staticEDs[BULK_LIST_HEAD]);

	USB_REG(HostID)->HcHCCA = (uint32_t) &(ohci_data[HostID].hcca);	/* Hook Hcca */

	/* Set up HcControl */
	USB_REG(HostID)->HcControl |= CONTROL_BULK_SERVICE_RATIO |
	...
	/* Set Global Power */
	USB_REG(HostID)->HcRhStatus = HC_RH_STATUS_LocalPowerStatusChange;

	// HcInterrupt Registers Init
        tem = USB_REG(HostID)->HcInterruptStatus;        // just to clear warning
	USB_REG(HostID)->HcInterruptStatus |= tem;	/* Clear Interrupt Status */
	USB_REG(HostID)->HcInterruptDisable = HC_INTERRUPT_ALL;/* Disable all interrupts */
	/* Enable necessary Interrupts */
	USB_REG(HostID)->HcInterruptEnable = HC_INTERRUPT_MasterInterruptEnable | HC_INTERRUPT_WritebackDoneHead |
...
{
	USB_REG(HostID)->HcCommandStatus = HC_COMMAND_STATUS_HostControllerReset;
	while ( USB_REG(HostID)->HcCommandStatus & HC_COMMAND_STATUS_HostControllerReset) {}	/* FIXME Wait indefinitely (may need a time-out here) */
...
{
	USB_REG(HostID)->HcControl =
		(USB_REG(HostID)->HcControl & (~HC_CONTROL_HostControllerFunctionalState)) | (HC_HOST_OPERATIONAL << 6);
	return HCD_STATUS_OK;
}
...
{
	USB_REG(HostID)->HcControl =
		(USB_REG(HostID)->HcControl & (~HC_CONTROL_HostControllerFunctionalState)) | (HC_HOST_SUSPEND << 6);
	return HCD_STATUS_OK;
}

static __INLINE HCD_STATUS OHciRhPortPowerOn(uint8_t HostID, uint8_t uPortNumber)
{
	USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortPowerStatus;	/* SetPortPower */
	HcdDelayMS(2 * ( (USB_REG(HostID)->HcRhDescriptorA & HC_RH_DESCRIPTORA_PowerOnToPowerGoodTime) >> 24 ) );	/* FIXME need to delay here POTPGT */
...
{
	USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_LowSpeedDeviceAttached;	/* ClearPortPower */
	return HCD_STATUS_OK;
}
...
{
	if ( USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus) {	/* If device is connected */
		USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortSuspendStatus;/* SetPortSuspend */
	}
...
{
	if ( USB_REG(HostID)->HcRhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus) {	/* If port is currently suspended */
		USB_REG(HostID)->HcRhPortStatus1 = HC_RH_PORT_STATUS_PortOverCurrentIndicator;	/* ClearSuspendStatus */
	}
